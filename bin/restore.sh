...
docker compose -f $DOCKER_COMPOSE_FILE down || error_exit "Failed to stop containers."

# Удаляем старую директорию для восстановления, если она существует
if [[ -d "$TEMP_RESTORE" ]]; then
  rm -rf "$TEMP_RESTORE"
fi

# Создаём новую директорию для восстановления
echo "Creating restore directory..."
mkdir -p "${TEMP_RESTORE}" || error_exit "Failed to create restore directory."

# Распаковываем архив
echo "Extracting archive..."
tar -xzf "$BACKUP_ARCHIVE" -C "$TEMP_RESTORE" || error_exit "Failed to extract archive."

# Проверяем директорий и файлов для восстановления
if [[ ! -f "$TEMP_RESTORE/docker-compose.yml" ]]; then
  error_exit "Docker Compose file does not exist."
fi

if [[ ! -f "$TEMP_RESTORE/db_backup.sql" ]]; then
  error_exit "Database dump file does not exist."
fi

for dir in "${BACKUP_DIRS[@]}"; do
  full_path=$TEMP_RESTORE/$dir
  if [[ ! -d "$full_path" ]]; then
...
mv "$TEMP_RESTORE/docker-compose.yml" "$DOCKER_COMPOSE_FILE" || error_exit "Failed to restore Docker Compose file."

# Копируем файлы приложения
echo "Restoring directories..."
for dir in "${BACKUP_DIRS[@]}"; do
  rsync -av "$TEMP_RESTORE/$dir/" "$PROJECT_DIR/$dir" || error_exit "Failed to restore $dir directory."
done
...
cat "${TEMP_RESTORE}/db_backup.sql" | docker compose exec -T $DB_CONTAINER psql -U $DB_USER -d $DB_NAME >/dev/null 2>&1 || error_exit "Failed to restore database."
...
rm -rf "$TEMP_RESTORE" || error_exit "Failed to remove temporary restore directory."
